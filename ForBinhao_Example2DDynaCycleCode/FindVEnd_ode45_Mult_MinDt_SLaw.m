...
%tau_ext_end, external shear stress at each element at
% the end of this timestep, vector

%sigma_end, normal stress at each element at
% the middle of this timestep, vector

%v_end, slip rate at each element at
% the middle of this timestep, vector

%theta_end, state variable at each element at
% the end of this timestep, vector

%dt_, pre determined timestep length of this time step

function [tau_ext_end,sigma_end,v_end,slip_end,theta_end,MinDt] = FindVEnd_ode45_Mult_MinDt_SLaw(tau_ext_start,sigma_start,...
...
%     dt_now = min(abs(step_control_coef*Dc./v_trial));%giving an approximate time step
    ...
    % Quasi-dynamic
    [t_out,y_out] = ode45(@(t,y) RSmarching_RD_mult_SLaw(t,y,Kshear_matrix,Knormal_matrix,Dc,G,Cs,a,b,mu0,V0,Theta0,tau_rate,sigma_rate),...
